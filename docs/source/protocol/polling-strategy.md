...
2. **Lazy initialization:** In `readDefault()`, if `isBindingNodeInfo()` is
...
3. **Data arrives asynchronously** via BLE notify → `nodeInfoHandle()` →
...
A monolithic 1675-line function that maps `(deviceModel, protocolVer,
iotProtVer, specs, isHighVolt)` to a `DeviceFunction` with ~135 boolean flags.
