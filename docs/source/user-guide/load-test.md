...
1. Verify prerequisites (SOC >= 95%, no grid/PV input)
1. Poll every N seconds — logging SOC, voltage, current, AC/DC/PV/grid power,
   temperatures, device time estimate, and energy measurements
1. Warn if grid or PV charging is detected (> 10 W)
1. End when SOC reaches 0%, BLE connection drops, or you press Ctrl-C
1. Print a summary with capacity, average load, min voltage, and max temp
