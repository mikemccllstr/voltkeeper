# ABOUTME: AC200L mid-range portable — V1 protocol (minProtocolVer=1022 in Android).
# ABOUTME: NOTE(divergence): Unit 10 plan lists AC200L as V2Base, but the Android
# ABOUTME:   DeviceConnUtil.java sets minProtocolVer=1022 (<2000), which is V1.
# ABOUTME: TODO(AC200L): verify against hardware.
...
class AC200L(V1Base):
    ...
    def _build_control_struct(self):
        ...
        s.add_uint_field("eco_off_time", ECO_CONTROL + 1, unit="h")
        s.add_uint_field("eco_auto_off", ECO_AUTO_OFF, unit="h")
        s.add_uint_field("dc_eco_power", DC_ECO_POWER, unit="W")
        s.add_uint_field("ac_eco_power", AC_ECO_POWER, unit="W")
