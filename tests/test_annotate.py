...
from voltkeeper.annotate import (
    _annotation_index,
    _diff,
    _format_change,
    _load_or_init,
...
def test_save_is_atomic_preserves_old_file_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("voltkeeper.annotate.yaml.dump", boom)
    try:
...
def test_capture_baseline_marks_changing_bytes_as_volatile(monkeypatch):
    ...
    from voltkeeper.annotate import _capture_baseline

    # No real waiting between polls.
    monkeypatch.setattr("voltkeeper.annotate.asyncio.sleep", AsyncMock())
